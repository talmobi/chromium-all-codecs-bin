...
const downloadURLs = {
  ...
  win32: 'https://chromium.woolyss.com/f/chrlauncher-win32-stable-codecs-sync.zip',
  win64: 'https://chromium.woolyss.com/f/chrlauncher-win64-stable-codecs-sync.zip'
}
...
function existsAsync ( filePath ) {
  let fulfill = null
  const promise = new Promise( resolve => fulfill = x )
...
class BrowserFetcher {
  ...
  canDownload ( revision ) {
    ...
    let resolve
    const promise = new Promise( resolve => resolve = x )
  ...
  async remove ( revision ) {
    ...
    await new Promise( resolve => removeRecursive( folderPath, fulfill ) )
  }
...
function downloadFile ( url, destinationPath, progressCallback ) {
  ...
  const promise = new Promise( ( resolve, reject ) => { fulfill = x; reject = y } )
...
function extractZip ( zipPath, folderPath ) {
  return new Promise( ( resolve, reject ) => extract( zipPath, { dir: folderPath }, err => {
    if ( err )
    { reject( err ) }
    else
    { fulfill() }
